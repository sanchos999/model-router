#!/usr/bin/env python3
"""R14: DOCX + PDF generator for the Model Router manual (Router Control Center).

Reads docs/RUKOVODSTVO-R14.md, embeds fresh screenshots from state/e2e-shots,
writes:
  docs/Model_Router_v1_0_Rukovodstvo_R14.docx
  docs/Model_Router_v1_0_Rukovodstvo_R14.pdf
"""
import os
import re
import sys
...
MD = os.path.join(ROOT, "docs/RUKOVODSTVO-R14.md")
SHOTS = os.path.join(ROOT, "state/e2e-shots")
OUT_DOCX = os.path.join(ROOT, "docs/Model_Router_v1_0_Rukovodstvo_R14.docx")
OUT_PDF = os.path.join(ROOT, "docs/Model_Router_v1_0_Rukovodstvo_R14.pdf")

# section -> screenshot (fresh R14 browser-E2E shots)
SHOT_MAP = {
    "4. Обзор (Dashboard)": "r14_a_dashboard.png",
    "5. Провайдеры": "r14_b_providers.png",
    "7. Каталог моделей": "r14_c_models.png",
    "36. Как Router принимает решение (R14)": "r14_e_routing.png",
...
def build_docx() -> int:
    ...
    title.alignment = 1
    sub = doc.add_paragraph("Редакция R14 Router Control Center · 12.09.2026 · коммит fd14de4")
    sub.alignment = 1
...
def build_pdf() -> int:
    ...
    html = ['<h1 style="text-align:center">Model Router v1.0 — '
            ...
            '<p style="text-align:center">Редакция R14 Router Control Center · '
...
if __name__ == "__main__":
    ...
    sys.exit(0 if pages and n >= 8 else 1)
